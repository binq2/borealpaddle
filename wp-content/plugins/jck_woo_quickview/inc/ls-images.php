<?php global $post, $product, $woocommerce, $item, $pid; ?>

<?php if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly ?>
<?php
$prodImgs = array();
$i=0;
foreach($item->children() as $prod){
...
if(!empty($prodImgs)):
	$i = 0; foreach($prodImgs as $imgId => $imgData):
		if($i == 0):
			$html .= '<img src="'.$imgData['imgSrc'].'" data-'.$this->slug.'="'.implode(' ', $imgData['slideId']).'" class="'.$this->slug.'_image" data-rsTmb="'.$imgData['imgThumbSrc'].'">';
		else:
			$html .= '<a href="'.$imgData['imgSrc'].'" data-'.$this->slug.'="'.implode(' ', $imgData['slideId']).'" class="'.$this->slug.'_image rsImg" data-rsTmb="'.$imgData['imgThumbSrc'].'"></a>';
		endif;
	$i++; endforeach;
endif;
?>
