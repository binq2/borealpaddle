		...
		<table class="variations" cellspacing="0">
			<tbody>
				<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
					<tr>
						<td class="label"><?php echo wc_attribute_label( $name ); ?></td>
                            ...
                            if ( is_array( $options ) ) {
                                ...
                                if ( taxonomy_exists( sanitize_title( $name ) ) ) {
                                    ...
                                    foreach ( $terms as $term ) {
                                        ...
                                        $size_label = ( esc_attr( sanitize_title($name) ) == 'pa_size')? strtolower($term->name) : '';
                                        ...
                                        echo '<label for="'. esc_attr( sanitize_title($name) ) .'-' . strtolower($term->slug) . '" class="' . strtolower($term->slug) . ' '. esc_attr( sanitize_title($name) ) .'">'. $size_label .'</label>';
                                    }
                                } else {
