...
class OnOffField extends FormField{
	...
	public function setView(){
		...
		$this->_value .= '
		<div class="onoff '.((get_option($this->_name) != '') ? get_option($this->_name) : $default ).' " id="'.$this->_id.'_toggler">
			<div class="inner">
		...
		$this->_value .= '<script>
			jQuery("#'.$this->_id.'_toggler").onoffswitcher("#'.$this->_id.'");
		</script>';
...
class SelectField extends FormField{
	...
	public function setView(){
		...
		if (isset($options) && count($options)) {
			...
			$this->_value = '<div class="control-group">'.(($this->getLabel() != '') ? '<label class="control-label">'.$this->getLabel().'</label>' : '').'<div class="controls">
			<select id="'.$this->_id.'" name="'.$this->_name.'" '.(($required) ? ' required ' :  '' ).' >';
...
class TextareaField extends FormField{
	...
	public function setView(){
		...
		if (isset($size)) { $cols = $size[0]; $rows = $size[1]; } else { $cols = 20; $rows = 60; /* Default TextArea Size */ }
		if (!isset($required)) $required = false;
...
class PTPanel{
	...
	$page = add_menu_page($this->panelName, $this->panelName, 'administrator', 'pt-panel', array(&$this, 'loadPanel'));
